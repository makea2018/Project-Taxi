...
object Main_Object {
  def main(args: Array[String]): Unit = {
    ...
    // Final_parquet (Full)
    val full_parquet = get_full_parquet(parquet_0, parquet_1, parquet_2, parquet_3, parquet_4, "date")

    // Не сохраняет вообще ни в каком формате, создает много пустых папок с пустыми файлами. Не знаю, как решить данную проблему
    //save_to_parquet(full_parquet, "../results/result.parquet")
  ...
  def get_full_parquet(parquet_0: DataFrame, parquet_1: DataFrame, parquet_2: DataFrame, parquet_3: DataFrame, parquet_4: DataFrame, using_col: String):DataFrame = {
  ...
  // Сохранение таблицы в parquet формате
  def save_to_parquet(parquet_df: DataFrame, path: String):Unit = {
    // Все перепробовал, не сохраняет таблицу ни в каком формате, только папку создает, но в ней пустые папки с файлами
    parquet_df.write.parquet(path)
  }
}
